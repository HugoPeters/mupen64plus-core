...
static int savestates_load_m64p(struct device* dev, char *filepath)
{
    ...
    // fixme: figure out why gzopen isn't defined by zlib's 43583934 proxy defines
#if 0
                                ...
                                int (*read_func)(void *, void *, size_t))
{
    ...
    {
        main_message(M64MSG_STATUS, OSD_BOTTOM_LEFT, "State ROM header does not match current ROM. Game might crash!");
       /* free(savestateData);
        return 0;*/
    }
...
{
    return 16788288 + 1024 /* queue */ + 4 + 4096;
}
...
void savestates_save_to_buffer(const struct device* dev, char* outBuffer)
{
    ...
    tmpBuff[0] = (savestate_latest_version >> 24) & 0xff;
    tmpBuff[1] = (savestate_latest_version >> 16) & 0xff;
    tmpBuff[2] = (savestate_latest_version >> 8) & 0xff;
    tmpBuff[3] = (savestate_latest_version >> 0) & 0xff;
	PUTARRAY(tmpBuff, curr, unsigned char, 4);
...
{
    unsigned char* curr = (unsigned char*)inBuffer;

	if (strncmp((char*)curr, savestate_magic, 8) != 0)
	{
		main_message(M64MSG_STATUS, OSD_BOTTOM_LEFT, "State is not a valid Mupen64plus savestate.");
		return 0;
	}
...
static int savestates_save_m64p(const struct device* dev, char *filepath)
{
    ...
    save->size = savestates_get_buffer_size();
    ...
    savestates_save_to_buffer(dev, save->data);
